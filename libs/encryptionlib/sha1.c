    ...
    )
{
    ...
    encl_lib_memcpy( block, buffer, 64 );
    ...
    )
{
    ...
    {
        i = 64 - j;
        encl_lib_memcpy( &Context->Buffer[j], Buffer, i );
        TransformFunction(Context->State, Context->Buffer);
        for( ; i + 63 < BufferSize; i += 64 )
        {
            TransformFunction(Context->State, (uint8*)Buffer + i);
        }
        j = 0;
    }
    else
    ...
    encl_lib_memcpy( &Context->Buffer[j], &((uint8*)Buffer)[i], BufferSize - i );
}
    ...
    )
{
    ...
    Sha1Update( Context, (uint8*)"\x80", 1 );
    ...
    {
        Digest->bytes[i] = (uint8)((Context->State[i>>2] >> ((3-(i & 3)) * 8) ) & 255);
    }
}
