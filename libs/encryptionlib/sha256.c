    ...
    )
{
    ...
    while( BufferSize > 0 )
    {
        ...
        {
           TransformFunction( Context, (uint8*)Buffer );
           Context->length += BLOCK_SIZE * 8;
           Buffer = (uint8*)Buffer + BLOCK_SIZE;
           BufferSize -= BLOCK_SIZE;
        }
        else
        {
           n = MIN( BufferSize, (BLOCK_SIZE - Context->curlen) );
           encl_lib_memcpy( Context->buf + Context->curlen, Buffer, (uint32)n );
           Context->curlen += n;
    ...
    )
{
    ...
    // Append the '1' bit
    Context->buf[Context->curlen++] = (uint8)0x80;
    ...
    if( Context->curlen > 56 )
    {
        ...
        {
            Context->buf[Context->curlen++] = (uint8)0;
        }
        TransformFunction(Context, Context->buf);
        Context->curlen = 0;
    }
